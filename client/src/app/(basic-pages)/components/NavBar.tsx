...
export function NavigationMenuDemo() {
    return (
        <NavigationMenu className="h-20 flex justify-between items-center shadow-sm border-b sticky top-0 px-10 xl:px-16 w-screen bg-background">
            ...
            <Link href="/" passHref>
                <Image src={Logo} alt="logo" className="w-fit xl:h-16 h-14 object-contain" />
            </Link>
            ...
            <NavigationMenuList className="hidden md:flex gap-2 xl:gap-8 text-sm text-nowrap ">
                {NavMenuList.map((item, index) => (
                    <NavigationMenuItem key={index} >
                        <Link href={item.href} passHref className="hover:text-primary transition-colors">
                        ...
                        <SheetContent side={"right"}>
                            <SheetDescription>
                                ...
                                <ul className="list-none mt-10">
                                    {NavMenuList.map((item, index) => (
                                        <li key={index} className="flex gap-4 hover:text-primary transition-colors">
